...
public:
  ...
  // Get a row vector value from INI file as a set of doubles seperated by
  // space, returning default_value if not found.
  arma::rowvec GetVec(const std::string &name,
                      const arma::rowvec default_value) const;

  // Get a matrix value from INI file as a set of doubles seperated by space for
  // each column and seperated by ; or \n for each row, returning default_value
  // if not found.
  arma::mat GetMat(const std::string &name,
  ...
  // replace all occurrences of the "from" string with "to" string inside "str"
  // recursively
