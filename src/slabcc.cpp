...
int main(int argc, char *argv[])
{
	...
	if (is_active(verbosity::detailed_progress)) {
		cout << timing() << "Directional RMSE:" << V_error_planars << endl;
	}
	...
	if (is_active(verbosity::detailed_progress)) {
		cout << timing() << "Alignment term calculation grid point:" << ind2sub(as_size(slabcc_cell.grid), index_far) << endl;
	}
	...
	if (is_active(verbosity::intermediate_steps)) {
		...
		for (auto i = 0; i < charge_position.n_rows; ++i) {
			cout << "\t" << charge_position(i, slabcc_cell.normal_direction) * slabcc_cell.vec_lengths(slabcc_cell.normal_direction);
		}
		cout << endl;
	}
	...
	if (extrapol_slab) {
		...
		if (is_active(verbosity::detailed_progress)) {
			...
			cout << timing() << "Linear fit Root Mean Square Error: " << sqrt(linearfit_MSE) << endl;
