...
template <typename X> class Spline {
public:
  ...
  Spline(const arma::Row<X> &x, const arma::Col<X> &y) {
    ...
    if (x.n_elem < 3) {
      std::cerr
          << "Spline: Must have at least three points for interpolation!\n";
      return;
    }

    const arma::uword n = y.n_elem - 1;

    arma::Row<X> b(n), d(n), a(n), c(n + 1), l(n + 1), u(n + 1), z(n + 1),
        h(n + 1);
  ...
  X interpolate(const X &x) const {
    ...
    auto it =
        std::lower_bound(mElements.begin(), mElements.end(), element_type(x));
